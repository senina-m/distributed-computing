...
void bank_robbery(void *parent_data, local_id max_id) {
  for (int i = 1; i < max_id; ++i) {
    transfer(parent_data, i, i + 1, i+2);
  }
  if (max_id > 1) {
...
void print_history(const AllHistory *history) {
  ...
  int has_pending = 0;
  int nrows =
      history->s_history_len + 2; // 0 row (parent) is not used + Total row
  ...
  if (max_time > MAX_T) {
    fprintf(stderr,
            "print_history: max value of s_time: %d, expected s_time < %d!\n",
            max_time, MAX_T);
    return;
  }
  ...
  for (int i = 1; i <= history->s_history_len; ++i) {
    for (int j = 0; j <= max_time; ++j) {
      if (has_pending) {
        sprintf(buf, cell_format_pending, table[i][j].balance,
                table[i][j].pending);
      } else {
  ...
  const int underscrores =
      (first_column_width + 1) + (max_cell_width + 1) * (max_time + 1);
